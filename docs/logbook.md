...
| M7+ | "Prompt to Part" — pagina interattiva, download, accesso pubblico ON/OFF | — | — | 🟢 mergiata in `develop` ([#33](https://github.com/danielesalpietro/caliper-cad/pull/33), [#34](https://github.com/danielesalpietro/caliper-cad/pull/34), 2026-08-25, stesso giorno di M7) — estensione, non una milestone nuova. `services/dashboard/static/prompt-to-part.html`: prompt in linguaggio naturale → `/api/normalize` (L2.5/Ollama, vero) → `/api/generate` (rilancia `generate_and_verify.py` com'è, mai riscritto) → viewer 3D WebGL scritto da zero (parser STL binario + renderer minimale, niente librerie esterne) → 4 download (STEP, STL, G-code, PDF stile disegno tecnico via `reportlab`). **2 bug reali in più trovati**: la dashboard segnava Flowise "down" con `FLOWISE_PORT` non-default (`health_url` statica, mai esercitata contro una porta diversa dal default finché non l'abbiamo cambiata sul serio — PR #33, stesso PR del fix di persistenza Flowise `/root`→`/home/node`); "filament used"/"estimated printing time" nel G-code stanno *dopo* un dump di centinaia di righe di parametri del profilo, non nell'header — un tail a byte fissi li mancava sempre (PR #34). **Nuovo servizio** `slicer-watcher` (parallelo a `prusaslicer`, mai toccato: stesso binario, entrypoint diverso, comunica solo via volume condiviso — il binario PrusaSlicer non si può copiare nell'immagine dashboard, 19 librerie GTK/WebKit mancanti verificato dal vivo prima di scartare quella strada). **2 flag `.env` nuovi**: `PUBLIC_ACCESS` (Flowise/Open WebUI pubblici o solo interni — la dashboard non è mai gated, altrimenti un OFF da remoto si autoescluderebbe), `PROMPT_TO_PART_MODE` (RW/RO, gate lato server non solo UI). **Riserva dichiarata, non un'omissione**: `/api/generate` chiama GPT a pagamento senza controllo di accesso né limite di frequenza — tracciato in issue [#35](https://github.com/danielesalpietro/caliper-cad/issues/35). Verificato end-to-end dal vivo più volte sulla Z8, incluso un giro reale con prompt "foro filettato M6" → PASS, GO/NO-GO identici a M7, 4 file scaricati e inviati all'utente |
