...
| M4 | Chiusura del loop di retrieval, firewall simulato/fisico | [#5](https://github.com/danielesalpietro/caliper-cad/issues/5) | [logbook_fase4.md](logbook_fase4.md) | 🟡 implementata e verificata a mano (retry_log.jsonl esteso con `feature`/`spec_key`, `virtual_memory.py` con regola anti-bias corroborata da caso reale, Livello 7 esteso a due collezioni Qdrant separate) — non ancora mergiata in `develop`; nessun dataset Livello 6 reale né istanza Ollama/Qdrant viva in questa sessione, stesso limite di sandbox di M1–M3 |
