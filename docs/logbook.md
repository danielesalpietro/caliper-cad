...
| M6 | Bring-up reale su RunPod, prima esecuzione end-to-end viva | [#18](https://github.com/danielesalpietro/caliper-cad/issues/18) | [logbook_runpod_run0.md](logbook_runpod_run0.md), [logbook_fase6.md](logbook_fase6.md) | 🟡 non ancora mergiata — run0 (bring-up, branch `claude/m6-runpod-bringup-run0`): 3 bug reali chiusi (Flowise mai installato sull'immagine, `call_flowise_l2` con Structured Output Parser di Flowise 3.x, flag `--confirm` per Rischio #5), i due chatflow L2 mancanti dal M3 costruiti e versionati, causa plausibile del SIGSEGV isolata (256 vCPU visibili vs ~27 reali via cgroup); zero TC-E2E eseguiti, harvest rosso (nessuna generazione completata). run1 (branch `claude/m6-rerun-run1`, immagine con fix del run0 già mergiati): bootstrap Flowise automatico verificato (con 2 bug trovati: password generata da `start.sh` non sempre conforme, ordine bootstrap/import), policy SSRF di Flowise 3.x bloccante per L2.5→Ollama (fix runtime, proposto per il repo), E2E-1 rosso→verde (schema `tolerance_type` non ammetteva vuoto, ora fixato e ri-esportato), E2E-5/E2E-6 PASS puliti dal vivo (id Qdrant deterministici, gate anti-bias), E2E-3 meccanicamente corretto ma confuso dal SIGSEGV (non ancora applicato al servizio `verifier-executor` reale, solo validato in isolamento con `taskset`), E2E-7 non eseguibile (`GAUGE_CHECK_TIMEOUT_SECONDS` hardcoded, leva sbagliata nell'handoff); bench di 5 modelli per L2.5 (solo `gpt-4o-mini` a inventati=0%, `llama3.1:8b` migliore locale); **E2E-2/4/8/9 non tentati** — il fix SIGSEGV resta non collegato alla pipeline HTTP reale, blocco residuo principale per il prossimo run; primo harvest verde della milestone (`retry_log.jsonl` popolato da generazioni reali) |
